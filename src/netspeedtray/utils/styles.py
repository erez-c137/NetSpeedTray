...
from netspeedtray.constants.constants import (
    ConfigConstants, ColorConstants, UIStyleConstants, ComponentVisualConstants
)
...
def get_accent_color() -> QColor:
    ...
    try:
        ...
        return QColor(ConfigConstants.UI_ACCENT_FALLBACK)

def dialog_style() -> str:
    """Style for the main SettingsDialog."""
    dark_mode_active = is_dark_mode()
    sidebar_bg = UIStyleConstants.SETTINGS_PANEL_BG_DARK if dark_mode_active else UIStyleConstants.SETTINGS_PANEL_BG_LIGHT
    content_bg = UIStyleConstants.SETTINGS_PANEL_BG_DARK if dark_mode_active else UIStyleConstants.SETTINGS_PANEL_BG_LIGHT
    section_bg = "#2D2D2D" if dark_mode_active else "#F0F0F0"
    text_color = UIStyleConstants.SETTINGS_PANEL_TEXT_DARK if dark_mode_active else UIStyleConstants.SETTINGS_PANEL_TEXT_LIGHT
    ...
    return f"""
        ...
        QCheckBox::indicator {{
            border-radius: {ComponentVisualConstants.TOGGLE_TRACK_HEIGHT // 2}px;
        }}
...
def sidebar_style() -> str:
    ...
    dark_mode_active = is_dark_mode()
    sidebar_bg = UIStyleConstants.SETTINGS_PANEL_BG_DARK if dark_mode_active else ConfigConstants.UI_SIDEBAR_BG
    selected_bg = "#3A3A3A" if dark_mode_active else ConfigConstants.UI_SIDEBAR_SELECTED # This was an error, fixed.
    hover_bg = "#4A4A4A" if dark_mode_active else "#E0E0E0"
    text_color = UIStyleConstants.SETTINGS_PANEL_TEXT_DARK if dark_mode_active else ConfigConstants.UI_TEXT_COLOR
...
def toggle_style(total_track_width: int, total_track_height: int) -> str:
    ...
    track_off_bg = getattr(UIStyleConstants, 'COMBOBOX_BORDER_DARK', '#333333') if dark_mode_active else getattr(UIStyleConstants, 'COMBOBOX_BORDER_LIGHT', '#CCCCCC')
    ...
    checkbox_text_color = UIStyleConstants.SETTINGS_PANEL_TEXT_DARK if dark_mode_active else UIStyleConstants.SETTINGS_PANEL_TEXT_LIGHT
...
def slider_style() -> str:
    ...
    groove_bg_inactive = getattr(UIStyleConstants, 'COMBOBOX_BORDER_DARK', '#333333') if dark_mode_active else "#E0E0E0"
...
def button_style(accent: bool = False) -> str:
    ...
    if accent:
        ...
        text_color = ColorConstants.WHITE
        ...
        text_color = ColorConstants.WHITE if dark_mode_active else ColorConstants.BLACK
...
def color_button_style(color_hex: str) -> str:
    ...
    if not (isinstance(color_hex, str) and color_hex.startswith("#") and len(color_hex) == 7):
        color_hex = ConfigConstants.DEFAULT_COLOR
    
    border_c = ConfigConstants.UI_BORDER_COLOR
    button_width = getattr(ConfigConstants, 'COLOR_BUTTON_WIDTH', 40)
    button_height = getattr(ConfigConstants, 'COLOR_BUTTON_HEIGHT', 20)
...
def always_dark_panel_style() -> Dict[str, str]:
    panel_background_color = UIStyleConstants.GRAPH_BG_DARK
    groupbox_title_text_color = UIStyleConstants.SETTINGS_PANEL_TEXT_DARK
    panel_border_color = UIStyleConstants.BORDER_COLOR
    
    combo_bg_dark_panel = UIStyleConstants.COMBOBOX_BG_DARK
    combo_border_dark_panel = UIStyleConstants.COMBOBOX_BORDER_DARK
    ...
    styles = {
        ...
        "QLabel_PanelDark": f"color: {UIStyleConstants.SETTINGS_PANEL_TEXT_DARK}; background-color: transparent; border: none; outline: none; font-size: 13px; font-family: 'Segoe UI Variable';",
        "QComboBox_PanelDark": f"""
            QComboBox {{ 
                background-color: {combo_bg_dark_panel}; 
                color: {UIStyleConstants.SETTINGS_PANEL_TEXT_DARK}; 
            ...
            QComboBox QAbstractItemView {{
                ...
                selection-color: {ColorConstants.WHITE}; 
                border: 1px solid {combo_border_dark_panel};
                border-radius: 4px; 
                outline: none;
            }}
