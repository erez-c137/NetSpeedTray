...
- **The readout jumped sideways when CPU/GPU load crossed a digit (#179).** In the side-by-side layout the whole readout is anchored to the tray edge, and that anchor was measured from the live text width, so a CPU or GPU percentage ticking from 9 to 10 (or 99 to 100) nudged the entire block, including the network speed to its left. The percentage now renders in a fixed-width field, so it holds still.
