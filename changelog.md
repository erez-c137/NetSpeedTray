...
- **Preferred Monitor was ignored on multi-monitor setups (#72).** NetSpeedTray only ever found the primary taskbar: it required every taskbar to have a system-tray/clock area, which on Windows 11 only the primary taskbar has, so every secondary-monitor taskbar was silently discarded and the widget always landed on the primary no matter which monitor you picked. It now recognizes secondary-monitor taskbars, so Preferred Monitor can place the widget on the monitor you choose, backed by more robust name-and-geometry matching and clearer diagnostics in the Support Bundle. (Thanks to @Mythos for the detailed bundle that pinned it down.)
