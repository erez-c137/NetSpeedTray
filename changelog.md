...
- **Native Windows 11 window chrome:** The Settings, Graph, and App Activity windows now have a **dark title bar** that follows your theme and **rounded corners**, via the Desktop Window Manager — so they read as part of Windows 11 rather than generic windows. (Silently does nothing on Windows 10.)
...
- **Quick actions in the tray menu:** The right-click menu now has a **Pause / Resume** toggle to stop and restart monitoring on demand, and a **"data used today / this month"** glance pinned to the top so you can read your totals at a glance without opening the graph.
...
- **First-run tour & a self-describing tray menu:** A brand-new install gets a one-time flyout pointing at the half of the app most people never find (graphs, per-app activity, hardware stats). The right-click menu is now a four-tier map with clickable usage rows that open the history graph, a self-describing "Hardware monitor: On/Off" row, and a "Show me around" entry to re-open the tour. A hover hint teaches the right-click/double-click gestures over the first few sessions, then goes quiet.
...
- **Arrow styles:** Five curated, native glyph presets — Classic (↑↓), Solid (▲▼), Compact (▴▾), Outline (△▽), Double (⇑⇓) — plus a Custom option, picked from a Windows-11 segmented control in Settings. Classic follows your OS language's default arrow.
...
- **Reset & reduce-motion:** A new **Advanced** settings page adds data-retention control, an app-wide "reduce motion" flag, and "Reset this page" / "Reset all to defaults" (your saved history is kept).
...
### Changed
- **App Activity is now honest about what it measures:** The window used to show per-app "Download/Upload speed" estimated from each process's total I/O (disk *and* network combined) — Windows can't attribute network bytes per-app without admin rights, so that number was misleading. It now reports only what it can measure exactly and for free: the live network **connections** each app holds (total, how many are active, and the distinct remote hosts they're reaching), rolled up per program. No estimates dressed up as speed.
...
- **More strings localized:** The history-graph window title and its "Mbps" stat labels, several error/dialog titles, the colors-page "Mbps" suffix, the color-picker title, and the App Activity Remote-Desktop message now go through translation instead of hardcoded English. The new 2.0 surfaces — the tray "data used" glance, the update-notes prompt, and the first-run welcome — added their own keys across all 10 languages (AI-assisted for the non-English values, pending native review, per `TRANSLATORS.md` and issue #157).
...
- **Settings design-system foundation:** A shared visual-token layer (a Fluent type ramp + semantic light/dark surface colors in `constants/styles.py`, consumed via `utils/styles.font()` / `semantic_colors()`), four reusable Fluent primitives (`SettingCard`, `SettingExpander`, `Win11Segmented`, `ColorField`), and a `utils/dwm.py` wrapper for the native window chrome. This is the groundwork (shared with the planned unified Monitor window) for the larger settings UI refresh.
...
- **Shared widget paint path + PreviewWidget (the keystone):** The widget's paint logic was lifted into one pure render function driven by a metrics snapshot, and a standalone `PreviewWidget` now draws through it. The live widget and every preview (the Settings live-preview, the planned Monitor window) render through the same code, so a preview matches the real widget exactly. The settings apply-path was also split so a commit no longer rebuilds the renderer twice.
