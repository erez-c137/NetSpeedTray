...
- **Update prompts now show the release notes:** When an update is available, the prompt shows what changed inline (as read-only text) with **Download** and **Skip this version** buttons, instead of only linking out to the browser. (This is the first step toward one-click auto-update; downloading and verifying the signed installer in-app follows in a later release.)
