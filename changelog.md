...
- **Quick actions in the tray menu:** The right-click menu now has a **Pause / Resume** toggle to stop and restart monitoring on demand, and a **"data used today / this month"** glance pinned to the top so you can read your totals at a glance without opening the Monitor.
...
- **First-run tour & a self-describing tray menu:** A brand-new install gets a one-time flyout pointing at the half of the app most people never find (the history graph, per-app activity, hardware stats — now all in the Monitor). The right-click menu is now a self-describing map with clickable usage rows that open the Monitor, a "Hardware monitor: On/Off" row, and a "Show me around" entry to re-open the tour. A hover hint teaches the right-click/double-click gestures over the first few sessions, then goes quiet.
...
- **Honest statistics + export engine:** A tier-aware `WindowSummary` engine computes exact percentiles only from the 24-hour raw tier and reports avg+max (percentiles "unavailable") for the rolled-up minute/hour tiers, with coverage and sample counts — never fabricating precision. A two-file CSV+JSON exporter and a headless `--export-csv` CLI run it; the CLI opens `WidgetState` in a new read-only mode (no writer thread, no timers) so it can export without a running app. Temperature/power/latency are persisted as **unclamped** hardware stat-types (the old 0–100 clamp was correct only for utilization).
